      ...
      <body style={{ margin: 0, fontFamily: "'Apple SD Gothic Neo', 'Malgun Gothic', sans-serif", background: "#f0f2f8", display: "flex", minHeight: "100vh" }}>
        <aside style={{ width: "210px", minHeight: "100vh", background: "linear-gradient(180deg, #1e3a5f 0%, #16294a 100%)", color: "#fff", display: "flex", flexDirection: "column", flexShrink: 0, boxShadow: "2px 0 12px rgba(0,0,0,0.15)" }}>
          <div style={{ padding: "28px 20px 20px" }}>
            ...
            <div style={{ fontSize: "11px", color: "#7fb3e8", marginTop: "5px", fontWeight: 400 }}>보험영업 AI 툴킷</div>
          </div>
          ...
          <nav style={{ flex: 1, padding: "16px 10px" }}>
            {NAV.map(({ href, label, icon }) => (
              <Link
                key={href}
                href={href}
                style={{ display: "flex", alignItems: "center", gap: "10px", padding: "10px 14px", color: "#b8d4f0", textDecoration: "none", fontSize: "14px", borderRadius: "8px", marginBottom: "4px", transition: "all 0.15s" }}
                onMouseEnter={e => { e.currentTarget.style.background = "rgba(255,255,255,0.1)"; e.currentTarget.style.color = "#fff"; }}
                onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#b8d4f0"; }}
              >
