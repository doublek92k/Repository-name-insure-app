...
export default function Documents() {
  ...
  async function addDoc() {
    ...
    await supabase.from("documents").insert([{ customer: form.customer, type: form.type, memo: form.memo, file_url: fileUrl || null, file_name: fileName || null }]);
  ...
  const TYPE_COLOR: Record<string, string> = { "청약서": "#ede9fe", "설계서": "#dbeafe", "동의서": "#d1fae5", "계약서": "#fef3c7", "기타": "#f1f5f9" };
  const TYPE_TEXT: Record<string, string> = { "청약서": "#7c3aed", "설계서": "#2563eb", "동의서": "#059669", "계약서": "#d97706", "기타": "#64748b" };
    ...
    <div style={{ maxWidth: "800px" }}>
      ...
      <div style={{ background: "#fff", borderRadius: "14px", padding: "20px", boxShadow: "0 1px 6px rgba(0,0,0,0.06)", border: "1px solid #f1f5f9", marginBottom: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: "10px", marginBottom: "10px" }}>
          ...
          <select value={form.type} onChange={(e) => setForm((f) => ({ ...f, type: e.target.value }))} style={{ border: "1px solid #e2e8f0", borderRadius: "8px", padding: "9px 12px", fontSize: "14px" }}>
            {DOC_TYPES.map((t) => <option key={t} value={t}>{t}</option>)}
          </select>
          <input placeholder="메모 (선택)" value={form.memo} onChange={(e) => setForm((f) => ({ ...f, memo: e.target.value }))} style={{ border: "1px solid #e2e8f0", borderRadius: "8px", padding: "9px 12px", fontSize: "14px" }} />
        </div>
        ...
        <button onClick={addDoc} disabled={adding} style={{ background: "#4f46e5", color: "#fff", border: "none", borderRadius: "8px", padding: "10px 20px", fontSize: "14px", fontWeight: 700, cursor: "pointer" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        ...
        {docs.map((doc) => (
          <div key={doc.id} style={{ background: "#fff", borderRadius: "12px", padding: "16px 20px", boxShadow: "0 1px 4px rgba(0,0,0,0.05)", border: "1px solid #f1f5f9", display: "flex", alignItems: "center", gap: "14px", flexWrap: "wrap" }}>
            ...
            {doc.file_url && <a href={doc.file_url} target="_blank" rel="noopener noreferrer" style={{ background: "#f0fdf4", color: "#059669", border: "1px solid #bbf7d0", borderRadius: "8px", padding: "5px 12px", fontSize: "12px", fontWeight: 600, textDecoration: "none", flexShrink: 0 }}>⬇ {doc.file_name || "파일"}</a>}
