...
const STATUS_COLOR: Record<string, string> = { "상담중": "#ede9fe", "이탈위험": "#fee2e2", "갱신임박": "#fef3c7", "성약완료": "#d1fae5", "휴면": "#f1f5f9" };
const STATUS_TEXT: Record<string, string> = { "상담중": "#4f46e5", "이탈위험": "#dc2626", "갱신임박": "#d97706", "성약완료": "#059669", "휴면": "#64748b" };
...
export default function CustomerDetail() {
  ...
  async function saveCustomer() {
    ...
    setSaving(false); setEditing(false); fetchCustomer();
  }
  ...
  return (
    <div style={{ maxWidth: "720px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "24px", flexWrap: "wrap" }}>
        ...
        <span style={{ background: STATUS_COLOR[customer.status] || "#f1f5f9", color: STATUS_TEXT[customer.status] || "#64748b", borderRadius: "20px", padding: "4px 12px", fontSize: "13px", fontWeight: 600 }}>{customer.status}</span>
        <button onClick={() => setEditing(!editing)} style={{ background: editing ? "#f1f5f9" : "#4f46e5", color: editing ? "#64748b" : "#fff", border: "none", borderRadius: "8px", padding: "8px 16px", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>{editing ? "취소" : "✏️ 수정"}</button>
        <button onClick={deleteCustomer} style={{ background: "#fee2e2", color: "#dc2626", border: "none", borderRadius: "8px", padding: "8px 16px", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>🗑️ 삭제</button>
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: "14px", padding: "24px", boxShadow: "0 1px 6px rgba(0,0,0,0.06)", border: "1px solid #f1f5f9", marginBottom: "20px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: "14px" }}>
          {[{ label: "이름", key: "name" }, { label: "나이", key: "age" }, { label: "직업", key: "job" }, { label: "전화번호", key: "phone" }].map(({ label, key }) => (
            <div key={key}>
              <div style={{ fontSize: "11px", color: "#94a3b8", fontWeight: 600, marginBottom: "4px", textTransform: "uppercase" }}>{label}</div>
              {editing ? <input value={form[key] || ""} onChange={(e) => setForm((f: any) => ({ ...f, [key]: e.target.value }))} style={{ width: "100%", border: "1px solid #e2e8f0", borderRadius: "8px", padding: "8px 10px", fontSize: "14px", boxSizing: "border-box" }} />
          ...
          <div>
            ...
            {editing ? <select value={form.status || ""} onChange={(e) => setForm((f: any) => ({ ...f, status: e.target.value }))} style={{ width: "100%", border: "1px solid #e2e8f0", borderRadius: "8px", padding: "8px 10px", fontSize: "14px" }}>{STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}</select>
          ...
          <div>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: "8px", flexWrap: "wrap" }}>
              {editing ? <input type="number" min={0} max={100} value={form.score || 0} onChange={(e) => setForm((f: any) => ({ ...f, score: Number(e.target.value) }))} style={{ width: "80px", border: "1px solid #e2e8f0", borderRadius: "8px", padding: "8px 10px", fontSize: "14px" }} />
              ...
              {editing && <button onClick={applyAutoScore} style={{ background: "linear-gradient(90deg,#4f46e5,#7c3aed)", color: "#fff", border: "none", borderRadius: "8px", padding: "8px 14px", fontSize: "12px", fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>✨ 자동계산</button>}
              {scorePreview !== null && editing && <span style={{ fontSize: "12px", color: "#7c3aed", fontWeight: 600 }}>→ {scorePreview}점</span>}
            </div>
            {editing && <div style={{ fontSize: "11px", color: "#94a3b8", marginTop: "4px" }}>나이·직업·상태·메모 기반 자동계산</div>}
          </div>
        </div>
        <div style={{ marginTop: "14px" }}>
          <div style={{ fontSize: "11px", color: "#94a3b8", fontWeight: 600, marginBottom: "4px", textTransform: "uppercase" }}>메모</div>
          {editing ? <textarea value={form.memo || ""} onChange={(e) => setForm((f: any) => ({ ...f, memo: e.target.value }))} rows={3} style={{ width: "100%", border: "1px solid #e2e8f0", borderRadius: "8px", padding: "8px 10px", fontSize: "14px", boxSizing: "border-box", resize: "vertical" }} />
            : <div style={{ fontSize: "14px", color: "#64748b" }}>{customer.memo || "메모 없음"}</div>}
        </div>
        {editing && <button onClick={saveCustomer} disabled={saving} style={{ marginTop: "16px", background: "#059669", color: "#fff", border: "none", borderRadius: "8px", padding: "10px 24px", fontSize: "14px", fontWeight: 700, cursor: "pointer", width: "100%" }}>{saving ? "저장 중..." : "💾 저장"}</button>}
      </div>
      ...
      <div style={{ background: "#fff", borderRadius: "14px", padding: "24px", boxShadow: "0 1px 6px rgba(0,0,0,0.06)", border: "1px solid #f1f5f9" }}>
        ...
        <div style={{ display: "flex", gap: "8px", marginBottom: "16px", flexWrap: "wrap" }}>
          <textarea value={newLog} onChange={(e) => setNewLog(e.target.value)} placeholder="상담 내용 입력..." rows={2} style={{ flex: 1, minWidth: "200px", border: "1px solid #e2e8f0", borderRadius: "8px", padding: "8px 10px", fontSize: "14px", resize: "none" }} />
